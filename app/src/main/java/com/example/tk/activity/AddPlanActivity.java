...
public class AddPlanActivity extends AppCompatActivity {
    ...
    private void savePlanToDatabase() {
        ...
        try {
            ...
            Log.d(TAG, "计划添加成功");
            Toast.makeText(this, "计划添加成功", Toast.LENGTH_SHORT).show();

            // 返回数据给UserMainActivity
            Intent resultIntent = new Intent();
            resultIntent.putExtra("date", selectedDate);
            setResult(RESULT_OK, resultIntent);
            finish();
        } catch (Exception e) {
            Log.e(TAG, "计划添加异常: " + e.getMessage());
            Toast.makeText(this, "计划添加异常: " + e.getMessage(), Toast.LENGTH_SHORT).show();
        }
    }
    @Override
